            ...
            if len(self.open):
                ...
                if extandState.degree >= self.MaxDegree:
                    ...
                    for i in range(len(self.spce)):
                        if (
                            (i == 0 and (spacIndex + self.spce[i]) >= 0)
                            or (
                                i == 1
                                and (spacIndex + self.spce[i])
                                < len(extandState.state) - 1
                            )
                            or (i == 2 and (spacIndex % self.length != 0))
                            or (i == 3 and ((spacIndex + 1) % self.length) != 0)
                        ):
                            ...
                            if state == self.targetNode.state:
                                ...
                                return True
                            elif not self.isInTable(
                                nodeState, self.close
                            ) and not self.isInTable(nodeState, self.open):
                                self.open.append(nodeState)
                                flag = True
                            else:
                                continue
