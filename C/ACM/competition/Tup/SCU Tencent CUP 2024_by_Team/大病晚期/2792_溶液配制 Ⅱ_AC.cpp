...
bool cmp(P a,P b){if(a.q()^b.q())return a.q()<b.q();return a.lef(b)>0;}
P a[N],b[N],p;
ll pw[N];
void solve(){
	int n,q;
	...
	while(q--){
		...
		for(int L=1,R=1;L <=tot;L++){
			if(L!=tot&&b[L].lef(b[L+1])==0&&b[L]*b[L+1]>0)continue;
			if(R<L)R=L;
			int f0=0,f1=0,cnt=0;
			for(int i=L;i>=0;i--){
				if(b[L].lef(b[i])==0&&b[L]*b[i]>0)cnt++;
				else break;
			}
			while(!f1&&R+1<=L+tot){
				R++;
