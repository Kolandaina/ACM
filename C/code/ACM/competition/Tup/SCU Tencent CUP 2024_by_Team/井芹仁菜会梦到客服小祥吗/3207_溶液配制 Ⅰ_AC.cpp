...
int main () {
	...
	for (int i = 0; i < q; ++i) {
		...
		lessCnt = power(2LL, less);
		bigCnt = power(2LL, bigger);
		if (equal) {
			eqCnt = power(2LL, equal);
		} else {
			eqCnt = 0;
		}
		//cout << lessCnt << " " << eqCnt << " " << bigCnt << endl;
		if (a[p - 1] != x) {
			ans = lessCnt * bigCnt - lessCnt - bigCnt + 1LL + mod;
			//cout << "ans: " << ans << endl;
			ans %= mod;
		} else {
			ans = ((eqCnt - 1)* lessCnt  % mod )* bigCnt % mod;
			ans += (lessCnt * bigCnt - lessCnt - bigCnt + 1LL + mod ) %mod;
			ans %= mod;
		}
		cout << ans << "\n";
	}
